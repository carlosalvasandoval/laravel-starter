...
class Datatable
{
    ...
    public function getData()
    {
        ...
        for ($i = 0; $i < count($searchColumns); $i++) {
            if (
                isset($_GET['columns'][$i]["search"]["value"]) && $_GET['search']["value"]
                != ''
            ) {
                $type                 = ($i != 0) ? " OR " : "";
        ...
        for ($i = 0; $i < count($this->where); $i++) {
            ...
            if (!empty($this->where[$i][1])) {
                ...
                if (
                    !is_numeric($this->where[$i][1]) && strlen($this->where[$i][1])
                    == 1
                ) {
        ...
        $new_query = DB::select($sql_all_data);

        if ($new_query[0]->found_rows > 0) {
            $iFilteredTotal = $new_query[0]->found_rows;
        } else {
